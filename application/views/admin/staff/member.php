...
<div id="wrapper">
                    ...
                    <div class="panel-body ">
                        ...
                        <div class="tab-content tw-mt-5">
                            <div role="tabpanel" class="tab-pane active" id="tab_staff_profile">
                                <?php echo form_hidden('default_language', 'english'); ?>
                                <?php echo form_hidden('is_not_staff', '0'); ?>
                                ...
                                $fullname_value = '';
                                if (isset($member)) {
                                    $fullname_value = trim($member->firstname . ' ' . $member->lastname);
                                }
                                $attrs = (isset($member) ? [] : ['autofocus' => true]); 
                                ?>
                                <?php echo render_input('fullname', 'Name', $fullname_value, 'text', $attrs); ?>
                                <?php echo form_hidden('firstname', ''); ?>
                                ...
                                <?php echo render_input('phonenumber', _l('staff_add_edit_phonenumber'), $value); ?>
                                <div class="form-group select-placeholder">
                                    <label for="company" class="control-label">Company</label>
                                    <?php $selected_company = (isset($member) && isset($member->company)) ? $member->company : ''; ?>
                                    <select name="company" id="company" class="selectpicker" data-width="100%"
                                        data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        <option value=""><?php echo _l('dropdown_non_selected_tex'); ?></option>
                                        <option value="Synergy Group (Group Employees only)" <?php echo ($selected_company == 'Synergy Group (Group Employees only)') ? 'selected' : ''; ?>>Synergy Group (Group Employees only)</option>
                                        <option value="Synergy Advertising" <?php echo ($selected_company == 'Synergy Advertising') ? 'selected' : ''; ?>>Synergy Advertising</option>
                                        <option value="Synite Digital" <?php echo ($selected_company == 'Synite Digital') ? 'selected' : ''; ?>>Synite Digital</option>
                                        <option value="Synchronize Media" <?php echo ($selected_company == 'Synchronize Media') ? 'selected' : ''; ?>>Synchronize Media</option>
                                        <option value="Syntax Communications" <?php echo ($selected_company == 'Syntax Communications') ? 'selected' : ''; ?>>Syntax Communications</option>
                                        <option value="BrandSynario/Synergyzer" <?php echo ($selected_company == 'BrandSynario/Synergyzer') ? 'selected' : ''; ?>>BrandSynario/Synergyzer</option>
                                    </select>
                                </div>
                                <?php 
                                $designation_value = '';
                                if (isset($member) && property_exists($member, 'designation')) {
                                    $designation_value = $member->designation;
                                }
                                ?>
                                <?php echo render_input('designation', 'Designation', $designation_value); ?>
                                <div class="form-group select-placeholder">
                                    <label for="department" class="control-label"><?php echo _l('staff_add_edit_departments'); ?></label>
                                    <?php 
                                    $selected_department = '';
                                    // First check if member has department column value
                                    if (isset($member) && isset($member->department) && !empty($member->department)) {
                                        $selected_department = $member->department;
                                    } elseif (isset($member) && isset($staff_departments) && count($staff_departments) > 0) {
                                        // Fallback to staff_departments table
                                        $selected_department = $staff_departments[0]['name'];
                                    }
                                    ?>
                                    <select name="department" id="department" class="selectpicker" data-width="100%"
                                        data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                                        <option value=""><?php echo _l('dropdown_non_selected_tex'); ?></option>
                                        <option value="Creative" <?php echo ($selected_department == 'Creative') ? 'selected' : ''; ?>>Creative</option>
                                        <option value="Design" <?php echo ($selected_department == 'Design') ? 'selected' : ''; ?>>Design</option>
                                        <option value="Strategy" <?php echo ($selected_department == 'Strategy') ? 'selected' : ''; ?>>Strategy</option>
                                        <option value="Production" <?php echo ($selected_department == 'Production') ? 'selected' : ''; ?>>Production</option>
                                        <option value="CS" <?php echo ($selected_department == 'CS') ? 'selected' : ''; ?>>CS</option>
                                        <option value="Tech" <?php echo ($selected_department == 'Tech') ? 'selected' : ''; ?>>Tech</option>
                                    </select>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <hr class="hr-10" />
                                        <?php echo form_hidden('administrator', '0'); ?>
                        ...
                        <div class="mtop15">
                            <table class="table dt-table" data-order-col="2" data-order-type="desc">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td width="50%">
                                            <div data-note-description="<?php echo e($note['id']); ?>" class="tw-whitespace-pre-wrap"><?php echo process_text_content_for_display($note['description']); ?></div>
    ...
    $(function() {
        ...
        // Initialize selectpicker for company and department dropdowns
        setTimeout(function() {
            if ($('select[name="company"]').length > 0 && !$('select[name="company"]').hasClass('bs-select-hidden')) {
                $('select[name="company"]').selectpicker();
            }
            if ($('select[name="department"]').length > 0 && !$('select[name="department"]').hasClass('bs-select-hidden')) {
                $('select[name="department"]').selectpicker();
            }
        }, 100);

        // Split fullname into firstname and lastname before form submission
        ...
        // Add asterisks to required field labels if not already present
        setTimeout(function() {
            var requiredFields = ['phonenumber', 'company', 'designation', 'department'];
            requiredFields.forEach(function(fieldName) {
                var label = $('label[for="' + fieldName + '"]');
                if (label.length > 0 && label.find('.req, .text-danger').length == 0) {
