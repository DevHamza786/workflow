...
return App_table::find('clients')
    ->outputUsing(function ($params) {
        ...
        $aColumns = [
            ...
            '(SELECT COUNT(id) FROM ' . db_prefix() . 'projects WHERE clientid = ' . db_prefix() . 'clients.userid) as totalProjects',
            '(SELECT COUNT(' . db_prefix() . 'tasks.id) FROM ' . db_prefix() . 'tasks INNER JOIN ' . db_prefix() . 'projects ON ' . db_prefix() . 'tasks.rel_id = ' . db_prefix() . 'projects.id WHERE ' . db_prefix() . 'tasks.rel_type = "project" AND ' . db_prefix() . 'projects.clientid = ' . db_prefix() . 'clients.userid) as totalTasks',
    ...
    <label class="onoffswitch-label" for="' . $aRow['userid'] . '"></label>
            ...
            // Total Jobs (Projects)
            $totalProjects = (int) ($aRow['totalProjects'] ?? 0);
            $row[] = '<span class="label label-default">' . $totalProjects . '</span>';

            // Total Tasks
            $totalTasks = (int) ($aRow['totalTasks'] ?? 0);
            $row[] = '<span class="label label-default">' . $totalTasks . '</span>';
            ...
            if (isset($row[2]) && is_string($row[2])) { // $row[2] contains the company HTML
                ...
                if (preg_match('/<div class="row-options">(.*?)<\/div>/is', $row[2], $matches)) {
                    ...
                    // Reconstruct row-options div
                    if (!empty($filteredLinks)) {
                        $newRowOptions = '<div class="row-options">' . implode(' | ', $filteredLinks) . '</div>';
                    } else {
                        $newRowOptions = '';
                    }
                    
                    // Replace the row-options div in the company HTML
