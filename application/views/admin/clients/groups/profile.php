...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <div class="tab-content mtop15">
                ...
                <div class="row">
                    ...
                    <div class="col-md-<?php echo !isset($client) ? 12 : 8; ?>">
                     ...
                     if (is_admin() || get_option('staff_members_create_inline_customer_groups') == '1') {
                        ...
                        */ ?>
                        <?php
                        // Find PKR currency ID for default value
                        $pkr_currency_id = '';
                        foreach ($currencies as $currency) {
                            // Search by currency name (ISO code like PKR) or symbol containing Rs
                            if (isset($currency['name']) && strtoupper(trim($currency['name'])) == 'PKR') {
                                $pkr_currency_id = $currency['id'];
                                break;
                            } elseif (isset($currency['symbol']) && stripos($currency['symbol'], 'Rs') !== false) {
                                // If name is PKR or symbol contains Rs, use it
                                if (empty($pkr_currency_id)) {
                                    $pkr_currency_id = $currency['id'];
                                }
                            }
                        }
                        
                        // Set default currency: use existing client currency, or PKR if found
                        $default_currency_value = '';
                        if (isset($client) && !empty($client->default_currency)) {
                            $default_currency_value = $client->default_currency;
                        } elseif (!empty($pkr_currency_id)) {
                            $default_currency_value = $pkr_currency_id;
                        }
                        
                        // Set default language: use existing client language, or 'english'
                        $default_language_value = '';
                        if (isset($client) && !empty($client->default_language)) {
                            $default_language_value = $client->default_language;
                        } else {
                            $default_language_value = 'english';
                        }
                        ?>
                        
                        <?php /* Hide Currency and Language fields, using hidden inputs with defaults */ ?>
                        <input type="hidden" name="default_currency" value="<?php echo e($default_currency_value); ?>">
                        <input type="hidden" name="default_language" value="<?php echo e($default_language_value); ?>">
            ...
            <?php /* Hide "Brand Admins" tab content
            <?php if (isset($client)) { ?>
