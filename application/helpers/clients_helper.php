...
function filter_client_visible_tabs($tabs, $id = '')
{
    ...
    // Only show Profile, Projects, and Tasks tabs
    $allowedTabs = ['profile', 'projects', 'tasks'];
    
    $appliedSettings = is_array($visible);
    foreach ($tabs as $key => $tab) {
        // Only show Profile, Projects, and Tasks
        if (!in_array($key, $allowedTabs)) {
            continue;
        }

        // Check visibility property from tab definition
...
function app_init_customer_profile_tabs()
{
    ...
    $CI->app_tabs->add_customer_profile_tab('profile', [
        ...
        'position' => 5,
        'badge'    => [],
        // Profile tab remains visible along with Projects and Tasks
    ]);
    ...
    $CI->app_tabs->add_customer_profile_tab('contacts', [
        ...
        'position' => 10,
        'badge'    => [],
        'visible'  => false, // Hide - only show Projects and Tasks
    ]);
