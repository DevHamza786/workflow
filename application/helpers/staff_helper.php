...
/**
 * Format staff data for dropdowns with combined name and department
 * @param  array $staff_array Staff array from staff_model->get()
 * @return array Formatted staff array with 'fullname' and 'department_name'
 */
function format_staff_for_dropdown($staff_array)
{
    $CI = & get_instance();
    ...
    foreach ($staff_array as $staff) {
        ...
        $staff_departments = $CI->departments_model->get_staff_departments($staff['staffid']);
        $department_name = '';
        if (count($staff_departments) > 0) {
            $department_names = array_column($staff_departments, 'name');
            $department_name = implode(', ', $department_names);
        }
