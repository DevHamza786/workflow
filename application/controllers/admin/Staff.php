...
class Staff extends AdminController
{
    ...
    public function member($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            // Remove fullname from data as it's not a database column
            unset($data['fullname']);
            
            // Handle department - store as department name string in the department column
            // Set empty departments array to avoid errors with staff_departments table
            // The department field will be saved as a string value (Creative, Design, etc.)
            if (!isset($data['departments'])) {
                $data['departments'] = [];
            }
            
            // Ensure company, designation, and department are properly set
            // These will be saved to the database columns
            ...
            // Handle email_signature only if it exists and is not empty
            $email_signature = $this->input->post('email_signature', false);
            if ($email_signature !== null && $email_signature !== '') {
                $data['email_signature'] = html_entity_decode($email_signature);
